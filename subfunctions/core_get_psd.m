...
function [P,scale]=core_get_psd(himt,density,Args,ii)
% 
...
P1 = cell(1,size(himt,1));

h = waitbar(0,['Please wait... processing image ',num2str(ii)]);

for i = 1:density:size(himt,1) %length(ic)
        ...
        P1{i}=var(twave,[],2).*exp(-(1/2)*((pi/2)*n/((length(scale)-1)/2)).^2);
        
%     end
    
    waitbar(i/size(himt,1),h)
